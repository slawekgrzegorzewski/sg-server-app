...
public class SyrFormatD extends SheetBasedSyrGeneratorStrategy {

    public SyrFormatD(Sheet sheet, List<Country> countries) {
        ...
        for (Row row : sheet) {
            ...
            if (isSecretCountriesRow(row)) {
                newSyr = new SecretCountriesSYR()
                        ...
                        .setAverageBibleStudies(getNumberFromCell(row, 9))
                        .setMemorialAttendance(getNumberFromCell(row, 10));
                result.add(newSyr);
                break;
            } else {
